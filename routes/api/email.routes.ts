...
import { ADMIN_ROLES, CLINICAL_ROLES } from '../../shared/auth/roles.js';
...
// The SMTP configuration surface is ADMIN only. Left open, any authenticated
// staff account could repoint outbound mail (host/port/user/password/from) at a
// server it controls and then drive `/send-appointments` through it. Reading
// `/config` is gated too even though the password is masked, since the rest of
// the row is the same secret in outline. The send endpoints stay open to any
// staff session — that is the front-desk workflow.
const adminOnly = authorize(ADMIN_ROLES);

// The send endpoints stay open to every staff role — mailing the day's schedule
// is a front-desk workflow — but the gate is written out so that "any role" is a
// decision on the page rather than the absence of one.
const staffOnly = authorize(CLINICAL_ROLES);
...
router.post(
  '/send-appointments',
  staffOnly,
  validate({ query: emailApi.sendAppointmentsQuery }),
  async (
...
This email was automatically generated on ${new Date().toLocaleString()}
      ...
      ErrorResponses.internalError(res, 'Failed to send appointment email', error as Error);
    }
  }
);
...
router.get(
  ...
  async (_req: Request, res: Response): Promise<void> => {
    try {
      ...
      ErrorResponses.internalError(res, 'Failed to read the email configuration', error as Error);
    }
  }
);
...
router.post(
  ...
  async (
    ...
    try {
      ...
      ErrorResponses.internalError(res, 'Failed to update the email configuration', error as Error);
    }
  }
);
 ...
 *
 * A POST, not a GET: it opens an outbound SMTP connection with the stored
 * credentials, so it is an action rather than a read — and as a GET it was
 * exempt from csurf while the session cookie is `sameSite: 'lax'`. ADMIN, like
 * the rest of the SMTP-configuration surface it probes.
 */
router.post(
  '/test',
  adminOnly,
  async (_req: Request, res: Response): Promise<void> => {
    try {
      ...
      ErrorResponses.internalError(res, 'Could not complete the email connection test', error as Error);
    }
  }
);
...
router.post(
  ...
  async (
    ...
    try {
      ...
      ErrorResponses.internalError(res, 'Failed to send the test email', error as Error);
    }
  }
);
