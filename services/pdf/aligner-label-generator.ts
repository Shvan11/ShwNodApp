...
class AlignerLabelGenerator {
  ...
  async generate(params: LabelGenerateParams): Promise<LabelResult> {
    ...
    if (
      ...
      startingPosition < 1 ||
      startingPosition > LABELS_PER_SHEET
    ) {
    ...
    const totalPages =
      totalLabels <= availableFirstPage
        ? 1
   ...
   * @private
   */
  private async _generatePdf(labels: LabelData[], config: LabelConfig): Promise<Buffer> {
  ...
  private _drawLabel(
    ...
    const totalTextHeight =
      logoHeight +
      logoSpacing +
      TYPOGRAPHY.FONTS.PATIENT_NAME +
      10 +
      TYPOGRAPHY.FONTS.DOCTOR_NAME +
      8 +
      6 +
      TYPOGRAPHY.FONTS.SEQUENCE;

    const startY = contentY + (contentHeight - totalTextHeight) * 0.35;
    let currentY = startY;

    // 1. Draw logo
    if (showLogo && config.logoPath) {
      try {
        const logoWidth = 60;
        ...
        log.warn('Failed to draw logo', { error: (error as Error).message });
      }
    ...
    doc
      .fontSize(TYPOGRAPHY.FONTS.SEQUENCE)
      .fillColor(TYPOGRAPHY.COLORS.TEXT)
      .font('Helvetica')
   ...
   * @private
   */
  private _drawBorder(doc: PDFKit.PDFDocument, position: Position): void {
    const { x, y } = position;
    doc
      .strokeColor(TYPOGRAPHY.COLORS.BORDER)
      .lineWidth(0.5)
