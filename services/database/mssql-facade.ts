/**
 * Database facade — mssql-backed, preserves public API for all query modules.
 *
 * This is the original SQL Server implementation. During the PostgreSQL migration
 * (Phase 3) it sits behind the driver dispatcher in `index.ts`: it is selected when
 * `DB_DRIVER=mssql` (the default). The PG counterpart is `pg-facade.ts`.
 */
