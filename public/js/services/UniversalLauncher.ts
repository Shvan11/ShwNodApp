 ...
 *
 * @example
 * import { UniversalLauncher } from '/js/services/UniversalLauncher';
 *
...
export class UniversalLauncher {
   ...
   *
   * @param appIdentifier - Application alias or full path
   * @param args - Array of arguments (will be pipe-separated)
   *
   * @example
   * // Using alias
   ...
   * ]);
   */
  static launch(appIdentifier: string, args: string[] = []): boolean {
    try {
      ...
      const encodedApp =
        appIdentifier.includes(':') || appIdentifier.includes('\\') || appIdentifier.includes('/')
          ? encodeURIComponent(appIdentifier)
          : appIdentifier;

      // Build the protocol URL
      const url =
        args.length > 0 ? `launch://${encodedApp}?args=${encodedArgs}` : `launch://${encodedApp}`;
   ...
   *
   * @param alignerBatchId - The aligner batch ID
   ...
   * UniversalLauncher.printAlignerLabels(193, 'Sibar Fathil', 1);
   */
  static printAlignerLabels(alignerBatchId: number, patientName: string, drId: number): boolean {
    ...
    return this.launch('msaccess', ['{AccessDatabase}', '/cmd', vbaCommandString]);
  }

  /**
   * Launch MS Access with a specific form and parameters
   *
   * @param formName - Name of the Access form
   ...
   * });
   */
  static openAccessForm(formName: string, params: Record<string, string | number> = {}): boolean {
    const args: string[] = ['{AccessDatabase}', '/cmd', formName];
   ...
   *
   * @param databasePath - Path to .accdb file (or use {AccessDatabase})
   *
   * @example
   * UniversalLauncher.openAccessDatabase('{AccessDatabase}');
   * UniversalLauncher.openAccessDatabase('C:\\MyDB\\data.accdb');
   */
  static openAccessDatabase(databasePath = '{AccessDatabase}'): boolean {
   ...
   */
  static openExcel(filePath: string): boolean {
   ...
   */
  static openWord(filePath: string): boolean {
   ...
   */
  static openNotepad(filePath: string): boolean {
   ...
   */
  static openFolder(folderPath: string): boolean {
   ...
   * );
   */
  static launchCustomApp(exePath: string, args: string[] = []): boolean {
 ...
 */
export function launchApp(appIdentifier: string, args: string[] = []): boolean {
