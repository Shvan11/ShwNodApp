// services/websocket-connection-manager.js
/**
...
class WebSocketConnectionManager {
  constructor() {
    ...
    // Track which client types are using this connection
    ...
    // Debounce rapid connection requests
    this.CONNECTION_DEBOUNCE_MS = 100;
  }

  /**
   * Ensure WebSocket is connected
   * @param {string} clientType - Type of client requesting connection
   * @param {Object} options - Additional connection options
   * @returns {Promise<WebSocketService>} - Connected WebSocket service
   */
  async ensureConnected(clientType, options = {}) {
    ...
    console.log(`[ConnectionManager] Starting new connection for client types:`, Array.from(this.clientTypes));
    ...
    this.connectionPromise = wsService.connect(connectionParams)
      .then(() => {
   ...
   * @param {string} clientType - Type of client disconnecting
   */
  removeClientType(clientType) {
   ...
   * @returns {WebSocketService} - WebSocket service
   */
  getService() {
    return wsService;
  }
