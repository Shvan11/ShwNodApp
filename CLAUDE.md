...
**Migrations:** owned by **node-pg-migrate** — `migrations/pg/*.sql` (plain SQL up/down; Kysely is the query builder, not the DDL owner). The Supabase mirror DDL lives in `migrations/supabase/`. Regenerate `types/db.d.ts` after any schema change. **Full procedures + guard rails: `docs/db-migrations.md` — read it before touching `migrations/pg/` or the `pgmigrations` ledger.**
- History was **squashed on 2026-07-30** to a single verified baseline (`1783100000000_baseline-2026-07-30.sql`, the 21 retired files live in git at `f83e10a`). New migrations must sort AFTER it.
- `change_log.changed_at` is stamped with **`clock_timestamp()`, not `now()`** (`1787000000000_cdc-capture-clock-timestamp.sql` + its Supabase mirror half). It is the drain engine's version token, and `now()` is `transaction_timestamp()` — shared by any two transactions that begin in the same microsecond, the one shape in which the guarded delete fails open and drops a change. Don't revert it to `now()`.
- **Never `--no-check-order`, and never re-run a baseline.** The ledger had drifted fully disjoint from disk (20 ghost rows vs 21 unrecorded files, one of them a full-schema baseline), so that flag would have replayed the whole schema over the live clinic DB. Three guard rails now block it: the baseline hard-refuses a database where `public.patients` exists, `npm run db:check` refuses to certify a drifted ledger and is wired as `predb:migrate` so `db:migrate` aborts before running, and `db:baseline:stamp` (the repair path — records the baseline as applied WITHOUT executing it) refuses to stamp an empty database.
- **Deploy order for DDL on a live server: build → restart the service → migrate.** Applying a drop while the running build still selects/inserts that column 500s the affected screens.
...
Boot **requires** (validated in `config/config.ts`, throws if missing): `MACHINE_PATH`, `SESSION_SECRET`, and the PostgreSQL block (`PG_HOST`/`PG_PORT`/`PG_DATABASE`/`PG_USER`/`PG_PASSWORD`, or `DATABASE_URL`). Both forms are resolved per-field in **`config/pg-connection.ts`** (discrete vars win over the URL) — `config.databasePg` is the SSoT every pg pool builds from, the app pool and the reverse CDC sink's own write pool alike, so anything that reads connection settings must go through it rather than re-reading `process.env`. `PORT` defaults to 3000. Optional service blocks (Telegram, Twilio, Google Drive, Google Contacts, WebCeph, Gemini, Supabase sync, LocalSend, Cloudflare Zero Trust list sync) disable when blank.

**Google OAuth integrations** (Drive = aligner-PDF storage; Contacts = message-recipient phone book) follow one pattern: browser redirect flow under `/api/admin/<provider>/{auth-url,callback}`, tokens in the LOCAL-ONLY `integration_oauth_tokens` table (never CDC-synced), status + disconnect under `/api/integrations/<provider>/*`, driven from Settings → Integrations. The pieces all three OAuth modules share (Drive, Contacts, 3Shape) live in **`utils/oauth.ts`** — `generateState`, `isInvalidGrantError`, `credentialsToTokenRow` — each provider re-exports them so its own module stays the single façade; **don't re-copy them** (a divergent `isInvalidGrantError` is how a revoked grant keeps reporting "Connected"). **`disconnect()` must clear the LIVE client too**, not just the DB row: `driveClient.loadStoredCredentials()` re-syncs in BOTH directions, and skipping that left the singleton uploading with a cleared token until the next restart. **Never** reintroduce `@google-cloud/local-auth` — it opens a browser + loopback consent server *on the server*, which cannot complete on the headless Windows service (that was the pre-2026-08-14 Contacts bug). Contacts differs from Drive in two ways: it is **multi-account** (registry `shared/google-contacts-accounts.ts`; token rows keyed `google_contacts:<accountId>`; the account rides in the session beside the CSRF state so the callback knows which grant the code is for — and since a consent screen can be *steered* by `login_hint` but never *forced*, the callback also reads the connected address back from `people/me`, stores it in `integration_oauth_tokens.account_email`, refuses a grant already claimed by another slot, and shows it on the Settings card, so a wrong-account connect is visible instead of silently serving the wrong phone book), and its client credentials are **resolved, not assumed** — explicit `GOOGLE_CONTACTS_*` env wins, else a pre-existing `credentials.json`, else the shared `GOOGLE_*` pair. That order is load-bearing: a refresh token is bound to the client that issued it, so an install carrying legacy `tokens/*.json` grants (auto-imported on first use) must keep using its credentials.json client or every refresh fails `invalid_client`. A desktop ("installed") client can refresh existing grants but cannot complete a browser sign-in; the Settings card detects this and says so. Legacy `DB_*` (SQL Server) vars feed only the Dolphin sink, not boot. `.env.example` documents every block by category; per-machine WSL overrides go in `.env.development`.
...
- **Every outbound HTTP call needs an explicit timeout.** `node-fetch` v3 dropped its `timeout` option, so a call without `signal: AbortSignal.timeout(ms)` has no upper bound: a *refused* connection fails fast, but a *dropped* packet (host asleep / firewalled / on another VLAN) hangs for the OS TCP timeout (~75s+), past Express's own 30s `requestTimeout` — so the client's friendly "is the workstation on?" message never fires. `utils/fetch-timeout.ts` (`isAbortError`/`describeFetchError`) maps the rejection onto that message; the 3Shape, WebCeph, LocalSend and Cloudflare clients all use it.
