...
- **Rollout is ratcheted + RTL is route-scoped.** Translated so far: Dashboard, UniversalHeader, the Settings General picker. RTL flips to `rtl` only when language is Arabic AND the route is in `RTL_ROUTES` (`core/language.ts`, currently `['/dashboard']`) — untranslated pages stay LTR so English is never mirrored. When you translate a screen, update three places in sync: the `eslint.config.js` `i18next/no-literal-string` file list (currently Dashboard + UniversalHeader), `RTL_ROUTES`, and the FOUC route check in `index.html`. Adding Kurdish (`ku`) = one `LANGUAGES` entry + `locales/ku/` + the FOUC language whitelist.
