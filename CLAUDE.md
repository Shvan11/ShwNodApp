...
- **Rollout is ratcheted + RTL is route-scoped.** Translated so far: Dashboard, UniversalHeader, the Settings General picker, and Expense Management (the `/expenses` route + its 5 expense components, `expenses` namespace). RTL flips to `rtl` only when language is Arabic AND the route is in `RTL_ROUTES` (`core/language.ts`, currently `['/dashboard', '/expenses']`) — untranslated pages stay LTR so English is never mirrored. When you translate a screen, update three places in sync: the `eslint.config.js` `i18next/no-literal-string` file list (currently Dashboard + UniversalHeader + the 6 expense files), `RTL_ROUTES`, and the FOUC route check in `index.html`. That ratchet's `jsx-attributes.exclude` also allowlists the shared modal's structural/identity attrs (`titleId`/`ariaLabelledBy`/`variant`) — non-user-facing, so leave them as literals (user-facing attrs `title`/`label`/`placeholder`/`alt`/`aria-label` stay checked). Adding Kurdish (`ku`) = one `LANGUAGES` entry + `locales/ku/` + the FOUC language whitelist.
